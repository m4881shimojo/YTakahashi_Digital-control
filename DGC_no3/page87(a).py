...
#Set poles of F(z),z1=0,z2=0,z3=0
#then a_1=0.;a_2=0,;a_3=0. 
aa1=0.;aa2=0.; aa3=0.0
a=(1/(p+q))*(aa1*q+aa3+q*(1+p-F1/(1+q))) #Eq,(a),(b) P86
Kgain=aa1+1+p-a #Eq,(c) P87
b=F1/Kgain/(1+q)-1 #Eq,(a),(b) (c)
Kc=Kgain/Kp
#print("a={:.3g},b={:.3g},K={:.3g},Kc={:.3g}".format(a,b,Kgain,Kc))
...
for i in range(0,knum):
    ...
    elif i>=ramp_start: r[i]=1.0+T*(i-ramp_start) #rump input
#
#
#-------------------------------------------------
#Calculate response #
...
for k in range(0,knum):
    ...
    #k時点での状態量の計算?　
    if k==0: y[k]=0.0#; print("if0y")
    elif k==1: y[k]=(1+p)*y[k-1]+Kp*u[k-1]#; print("if1y")
...
ax2.plot(t,u,drawstyle='steps-post',color='g', linestyle='dashed', marker='o')
plt.ylabel("Responce ")
plt.xlabel("step (k)")
